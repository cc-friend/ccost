...
Also includes `cctokens sl` — a subcommand that analyzes `~/.claude/statusline.jsonl` for **rate limit tracking**, session summaries, 5-hour budget estimation, and cost cross-comparison.
...
# Statusline analysis: rate limit timeline (default)
cctokens sl

# Session summaries with cost breakdown
cctokens sl --per session --cost decimal

# 5-hour budget estimation per window
cctokens sl --per window --cost decimal

# Rate limit chart over time
cctokens sl --chart 5h
```
...
## Statusline Analysis (`cctokens sl`)
...
Add a statusline hook to your Claude Code settings that appends to `~/.claude/statusline.jsonl`:
...
| Usage | Description |
|-------|-------------|
| `cctokens sl` | Rate limit timeline (default) — shows 5h% and 7d% changes over time |
| `cctokens sl --per session` | Session summaries — cost, duration, API time, API%, lines changed, segments |
| `cctokens sl --per project` | Project summaries — aggregated cost, duration, session count |
| `cctokens sl --per day` | Daily summaries — cost, sessions, peak rate limits |
| `cctokens sl --per window` | 5-hour window summaries — peak usage, estimated budget |
| `cctokens sl --chart 5h` | 5-hour rate limit percentage chart |
| `cctokens sl --chart 7d` | 7-day rate limit percentage chart |
| `cctokens sl --chart cost` | Cumulative cost chart |
...
All shared flags (`--from`, `--to`, `--tz`, `--session`, `--project`, `--model`, `--cost`, `--output`, `--filename`, `--copy`, `--order`, `--table`, `--5hfrom`, `--5hto`, `--1wfrom`, `--1wto`) work with `cctokens sl`.

### Segment-Aware Aggregation

When a Claude Code session is closed and resumed, cumulative counters (cost, duration, tokens, lines) reset to zero. `cctokens sl` detects these resets and correctly sums across segments:
...
The `--per window` view estimates total 5-hour budget from actual cost and rate limit percentage:

```
Est Budget = Cost in Window × 100 / Peak 5h%
```

If this number decreases over time, it may indicate quota reduction.
...
# Statusline: session summaries with cost
cctokens sl --per session --cost decimal

# Statusline: 5h window budget estimation
cctokens sl --per window --cost decimal

# Statusline: compare costs between statusline and LiteLLM
cctokens sl --per session --cost-diff --cost decimal
